...
# This script starts the DomiSMP server with HTTP support

echo "Starting DomiSMP Server with HTTP support..."
echo

# Set the configuration file location
SMP_CONFIG_FILE="./smp-config/smp.config.properties"

# Set Java options for SMP
JAVA_OPTS="-Xms512m -Xmx2048m"
JAVA_OPTS="$JAVA_OPTS -Dsmp.configuration.file=$SMP_CONFIG_FILE"
JAVA_OPTS="$JAVA_OPTS -Dspring.config.location=file:$SMP_CONFIG_FILE"
JAVA_OPTS="$JAVA_OPTS -Dspring.main.web-application-type=servlet"
JAVA_OPTS="$JAVA_OPTS -Djava.net.useSystemProxies=true"

# Add JVM options for better compatibility
